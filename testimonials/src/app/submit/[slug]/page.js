...
export default async function SubmitPage({ params }) {
  ...
  return (
    <main className="container narrow" style={{ '--accent': wall.accent }}>
      <header className="page-header">
        <h1>{wall.title}</h1>
        <p>{wall.description || 'We’d love to hear what you think. It takes a minute.'}</p>
      </header>
      <SubmitForm slug={wall.slug} wallTitle={wall.title} />
    </main>
  );
}
